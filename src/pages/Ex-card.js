...
export function ExCard() {
  ...
  const card = exercises[bodyPart].map((ex) => {
    return (
      <div
        className="bg-gradient-to-tr drop-shadow-lg  w-[400px] rounded-md overflow-y-auto 
            shadow-lg mb-3  dark:from-purple-400  dark:via-purple-700 dark:to-blue-400 from-blue-400 via-blue-900 to-purple-400 hover:shadow-xl transition-all duration-500 dark:shadow-slate-600 dark:hover:shadow-slate-600 p-3 h-auto "
        id="slide" key={ex.name}
        >
        <img
          className="flex flex-row w-full h-64 m-auto rounded-md shadow-xl mb-3 transition-all"
          src={ex.image}
          alt="muscolo"
        />
        <h1 className="font-bold text-xl pt-3 text-white">{ex.name}</h1>
        <h3 className="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-900 mr-2 my-2">
          Difficulty: {ex.difficulty}
        </h3>
        <div className="py-3">
          <p className=" text-gray-200 text-base h-72 overflow-y-auto scrollbar pr-2">{ex.instructions}</p>
        </div>
        <button className="drop-shadow-xl text-white bg-gradient-to-r rounded-md px-5 py-2  
            ...
            dark:to-blue-400 dark:from-violet-500 flex justify-center items-center w-full gap-3">
