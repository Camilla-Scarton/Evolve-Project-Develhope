...
function handlePosition() {
    ...
    return <div className='min-h-[500px] w-full h-full relative'>
        <Suspense fallback={null}>
            <Canvas shadows style={{ height: "550px" }} className='min-h-[500px] w-full h-full bg-white dark:bg-slate-900 justify-center items-center'>
                ...
                <PerspectiveCamera ref={camRef} position={[-1.2, 2.5, 7]} makeDefault />
                <OrbitControls ref={cameraRef} maxDistance={8} position={[0, 0, 0]} maxPolarAngle={1.9} enableZoom={false}/>
                <BodyModel position={[0, -3.5, 0]} triggerAnimation={triggerAnimation} rotation={[0, 3, 0]} castShadow scale={1.5} />
...
<span onClick={handlePosition} className="relative inline-block text-white">Zoom out</span>
</button>
    </div>
}